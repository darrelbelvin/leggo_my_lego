...
def draw_bbox(image, bboxes, classes, show_label=True, probability=True):
    """
    ...
    for i, bbox in enumerate(bboxes):
        ...
        bbox_thick = 2 # int(0.6 * (image_h + image_w) / 600)
        c1, c2 = (coor[0], coor[1]), (coor[2], coor[3])
        cv2.rectangle(image, c1, c2, bbox_color, bbox_thick)

        if show_label:
            bbox_mess = '%s: %.2f' % (classes[class_ind], score) if probability else classes[class_ind]
